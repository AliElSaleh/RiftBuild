...
read_only BuiltinOptionInfo BuiltinOptionsTable[26] =
{
    ...
    { .Short = {0},      .Long = SC("clean"),           .Description = SC("Remove all intermediate and output files generated by the current .build file."), .bAllowInBuildFile = true },
    { .Short = {0},      .Long = SC("clean_all"),       .Description = SC("Remove all intermediate and output files generated by all .build files that the current build file depends on."), .bAllowInBuildFile = true },
    { .Short = {0},      .Long = SC("rebuild"),         .Description = SC("Clean and rebuild the intermediate/output files defined by the current .build file."), .bAllowInBuildFile = true },
    { .Short = {0},      .Long = SC("rebuild_all"),     .Description = SC("Clean and rebuild all intermediate/output files across all .build files that the current build file depends on."), .bAllowInBuildFile = true },
    { .Short = {0},      .Long = SC("run"),             .Description = SC("After successfully building an executable, run it immediately."), .bAllowInBuildFile = true },
...
static bool IsOptionBuiltin(String Param)
{
    ...
    for (u8 i = 0; i < SArray_Capacity(BuiltinOptionsTable); i++)
    {
        ...
        {
            if (bAllowedInBuildFile)
            {
                // make an exception for these ones specifically
                continue;
            }
...
// build tree, and (3) require the path to live inside one of the directories the .build file itself
// declared as an output root: the build directory, the intermediate directory, or the compiler
// object directory (Compiler.ObjectDirectory, used by codegen modules that emit outside the build
// tree - their outputs must clean like any other artifact). ObjectBaseDirectory is null when the
// key was not declared. Deletion itself goes through Filesystem_DeleteFile (the Win32 DeleteFile
// API / unlink), never a shell, so a rejected-but-somehow-missed line still cannot execute a
// command, the checks are defense in depth.
static bool TryCleanFromManifest(const String IntermediateBaseDirectory, const String BuildBaseDirectory, const String ObjectBaseDirectory, const String BuildFileName)
{
    ...
    if (Filesystem_Open(ManifestPath, FileMode_Read, &f))
    {
        ...
        while (Filesystem_ReadLine(f, &Line))
        {
            ...
            if (bPathOK)
            {
                ...
                bool bInsideObjectDir    = ObjectBaseDirectory.Length > 0 && String_StartsWith(Path, ObjectBaseDirectory, false);
                if (!bInsideBuild && !bInsideIntermediate && !bInsideObjectDir)
                {
                        ...
                        BuildPlan* Plan, bool bPlanOnly)
{
    ...
    // "rebuild"/"clean" operate on this target only. The "_all" variants additionally propagate
    // down to every dependency (see the dependency loop below), so they clean/rebuild all targets.
    const bool bRebuildAll = StringArray_Contains(Parameters, S("rebuild_all"), false);
    const bool bCleanAll   = StringArray_Contains(Parameters, S("clean_all"), false);
    ...
    // if (!bExportingSomething)
    {
        ...
        for (u32 i = 0; !bSkipDependencies && i < Depends_Count; i++)
        {
            ...
            // Depends(private) stops them from bubbling any further than us (see bIsPrivateDependency).
            {
                    ...
                    FreshReceipt.AssemblyType == AssemblyType_StaticLibrary)
                {
                    ...
                    // On non-Windows platforms the build prepends "lib" to library file names
                    // (libchdr -> liblibchdr.a would be wrong for -l resolution), so remove the
                    // prefix from the propagated name. On Windows the artifact keeps the assembly
                    // name verbatim (libchdr -> libchdr.lib), so the name must not be trimmed or
                    // the linker is handed a file that does not exist (chdr.lib).
                    #if !PLATFORM_WINDOWS
    ...
    // Compiler.ObjectDirectory is a third output root (codegen modules emit outside the build
    // tree with it), so the manifest clean must accept paths under it. Null when not declared.
    StringArena(ObjectBaseDirectory, MAX_PATH_LENGTH, Arena);
    if (String_IsValid(CompilerObjectDirectory) && Filesystem_IsPathRelative(CompilerObjectDirectory))
    {
        String_BuildPath(&ObjectBaseDirectory, WorkingPath, CompilerObjectDirectory);
        String_AppendPathSeparator(&ObjectBaseDirectory);
        xx Filesystem_ConvertRelativeToAbsolutePath(&ObjectBaseDirectory);
    }
    ...
    */
    ...
    if (!TryRunBuildCommands(S("PostCompile"), WorkingPath, VariablesDB, &ExternalClock, bIsClean))
    {
        Receipt.ExitCode = 1;
        return Receipt;
    }
    ...
    if (bCanLink)
    {
        ...
        // postlink step
        if (!TryRunBuildCommands(S("PostLink"), WorkingPath, VariablesDB, &ExternalClock, bIsClean))
        {
            Receipt.ExitCode = 1;
            return Receipt;
        }
...
PostBuild:
    ...
    if (!TryRunBuildCommands(S("PostBuild"), WorkingPath, VariablesDB, NULL, bIsClean))
    {
        Receipt.ExitCode = 1;
        return Receipt;
    }
