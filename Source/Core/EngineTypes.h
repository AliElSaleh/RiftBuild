...
#define STRUCT(StructName) typedef struct StructName StructName; struct StructName
#define STRUCT_INHERIT(StructName) StructName CONCAT(StructName, _)
#define UNION(UnionName) typedef union UnionName UnionName; union UnionName

#define ENUM(EnumName) typedef u32 EnumName; enum
#define ENUM_TYPED(EnumName, Type) typedef Type EnumName; enum

/*
#define ENUM2(EnumName, ...) \
    typedef enum EnumName \
    {\
        __VA_ARGS__,\
        __##EnumName##_Count,\
        __##EnumName##_ForceU32 = 0x7fffffff\
    } EnumName
*/
...
#define each(Element, Array) (typeof(Array[0])* CONCAT(Element, _) = &Array[0], Element = *CONCAT(Element, _); CONCAT(Element, _) < &Array[Array_Num(Array)]; CONCAT(Element, _)++, Element = *CONCAT(Element, _))
#define each_i(Index, Element, Array) (typeof(Array[0])* CONCAT(Element, _) = &Array[0], Element = *CONCAT(Element, _); CONCAT(Element, _) < &Array[Array_Num(Array)]; CONCAT(Element, _)++, Element = *CONCAT(Element, _), ++(Index))
...
#elif defined(__PLAYDATE__)
    #define PLATFORM_PLAYDATE 1
    #define PLATFORM_STRING "Playdate"
#else
...
// CPU arch detection
    ...
    #define __CACHE_LINE_SIZE 64
        ...
        #define CPU_ARCHITECTURE_STRING "x86 x64"
    #else
    ...
    #define __CACHE_LINE_SIZE 64
        ...
        #define CPU_ARCHITECTURE_STRING "arm arm64"
        #define PLATFORM_64_BIT 1
    #else
...
//global f64 GClockFrequency;
#define nullptr nullptr_z
#endif

//FORCEINLINE void* Internal_NullAllocCallback(UNUSED u64 Size) { return nullptr; }
//FORCEINLINE void Internal_NullFreeCallback(UNUSED void* Memory) { }
