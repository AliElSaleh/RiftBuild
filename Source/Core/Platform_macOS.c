...
{
    return 0; // no UBR-equivalent on macOS
}

void Platform_GetDisplayVersion(String* OutVersion)
{
    UNUSED_PARAM(OutVersion); // no DisplayVersion-equivalent on macOS
}

void Platform_GetMachineId(String* OutId)
{
    UNUSED_PARAM(OutId); // TODO: IOPlatformUUID via IOKit
}

void Platform_GetDeviceId(String* OutId)
{
    UNUSED_PARAM(OutId); // no Settings "Device ID" equivalent on macOS
}
